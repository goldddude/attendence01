...
def create_vercel_app():
    ...
    # Health check — confirms MongoDB env var and connectivity
    @app.route('/health')
    def health():
        mongo_uri = os.getenv('MONGODB_URI', 'NOT SET')
        mongo_db  = os.getenv('MONGODB_DB', 'nfc_attendance')
        uri_preview = mongo_uri[:40] + '...' if len(mongo_uri) > 40 else mongo_uri
        try:
    ...
    def serve_static(path):
        # API routes are handled by blueprints above — don't intercept them
